...
export function RunsPage({ notify, activityRevision = "" }: { notify: (message: string, kind?: "success" | "error") => void; activityRevision?: string }) {
  ...
  return <div className="page-grid page-grid--runs">
    ...
    <main className="detail-pane">{loading ? <div className="skeleton-page" aria-label="正在调取运行卷宗"><i /><i /><i /></div> : !selected ? <EmptyState title="尚无运行卷宗">直接交办员工或签发一次 Workflow 后，这里会出现不可变的执行记录。</EmptyState> : <div className="dossier run-dossier">
      ...
      {selected.architecture === "supervisor" && <DossierSection number="02" title="动态执行图"><SupervisorRunTopology nodes={Object.values(selected.nodes)} /></DossierSection>}
      <DossierSection number={selected.architecture === "supervisor" ? "03" : "02"} title="节点结果"><div className="run-node-list">{Object.values(selected.nodes).map((node, index) => { const decision = supervisorDecision(node); return <article key={node.nodeId}><div className="run-node-head"><span className="node-number">{String(index + 1).padStart(2, "0")}</span><div><strong>{node.nodeId}</strong><code>{node.roleId}{node.metadata?.kind === "supervisor" ? ` · 领队 Round ${node.metadata.round ?? "—"}` : node.metadata?.kind === "member" ? ` · 成员 Round ${node.metadata.round ?? "—"}` : ""}</code></div><Stamp status={node.status} /></div><dl className="ledger horizontal"><dt>尝试</dt><dd>{node.attempts}</dd><dt>开始</dt><dd>{formatTime(node.startedAt)}</dd><dt>结束</dt><dd>{formatTime(node.completedAt)}</dd></dl>{decision && <div className="supervisor-decision-summary"><code>{decision.action.toUpperCase()}</code><span>{decision.summary ?? "领队未提供本轮摘要。"}</span></div>}{node.error && <div className="inline-error">{node.error}</div>}{node.output !== undefined && <pre className="result-json">{JSON.stringify(node.output, null, 2)}</pre>}<code className="artifact-path">{node.artifactDir}</code></article>; })}</div></DossierSection>
      {selected.output !== undefined && <DossierSection number={selected.architecture === "supervisor" ? "04" : "03"} title="Workflow 最终输出">{finalSummary(selected) && <p className="workflow-final-summary">{finalSummary(selected)}</p>}<pre className="result-json">{JSON.stringify(selected.output, null, 2)}</pre></DossierSection>}
    </div>}</main>
  </div>;
}
