      ...
      <fieldset className="daemon-write-surface" disabled={!daemonAvailable}>
        ...
        <section className="workflow-contract"><div className="section-kicker"><b>02</b><span>固定目标</span></div>
          <div className="form-grid entrance-target-grid">
            <Field label="Direct 目标"><SelectControl ariaLabel="选择 Direct 目标" value={draft.directMode} options={[{ value: "caller", label: "交还调用方", description: "不创建内部工单或运行" }, { value: "employee", label: "固定 Employee", description: "创建单员工工单与 Run" }, { value: "none", label: "不配置 Direct", description: "只允许规则或兜底结果决定去向" }]} onChange={(directMode) => setDraft({ ...draft, directMode: directMode as DirectMode })} /></Field>
            {draft.directMode === "employee" && <Field label="Direct Employee"><SelectControl ariaLabel="选择 Direct Employee" value={draft.directEmployeeId} invalid={!draft.directEmployeeId} options={[{ value: "", label: "选择员工", disabled: true }, ...activeEmployees.map((employee) => ({ value: employee.id, label: employee.identity.displayName, description: `${employee.id} · v${employee.version}` }))]} onChange={(directEmployeeId) => setDraft({ ...draft, directEmployeeId })} /></Field>}
            <Field label="Leader 协作编排"><SelectControl ariaLabel="选择 Leader 协作编排" value={draft.leaderWorkflowId} options={[{ value: "", label: "不配置 Leader", description: "团队协作入口不可用" }, ...supervisorWorkflows.map((workflow) => ({ value: workflow.id, label: workflow.id, description: `固定 v${workflow.version}` }))]} onChange={(leaderWorkflowId) => setDraft({ ...draft, leaderWorkflowId })} /></Field>
          </div>
          ...
          <div className="entrance-specialist-editor">
            {draft.specialists.map((row, index) => <article key={row.clientId} className={specialistIssues[index] ? "is-invalid" : ""}>
              ...
              <Field label="目标类型"><SelectControl ariaLabel={`选择专家目标 ${row.key || index + 1} 的类型`} value={row.kind} options={[{ value: "project-role", label: "Project Role", description: "通过固定项目角色调用员工" }, { value: "employee", label: "Employee", description: "直接固定一位员工" }, { value: "graph-workflow", label: "Graph Workflow", description: "调用固定 Graph 编排" }]} onChange={(kind) => updateSpecialist(row.clientId, { kind: kind as SpecialistKind, referenceId: "", pinned: undefined })} /></Field>
              <Field label="固定目标"><SelectControl ariaLabel={`选择专家目标 ${row.key || index + 1} 的固定目标`} value={row.referenceId} invalid={Boolean(specialistTargetIssues[index])} options={row.kind === "employee" ? [{ value: "", label: "选择员工", disabled: true }, ...activeEmployees.map((employee) => ({ value: employee.id, label: employee.identity.displayName, description: `${employee.id} · v${employee.version}` }))] : row.kind === "project-role" ? [{ value: "", label: "选择项目角色", disabled: true }, ...projectRoleOptions] : [{ value: "", label: "选择 Graph", disabled: true }, ...graphWorkflows.map((workflow) => ({ value: workflow.id, label: workflow.id, description: `固定 v${workflow.version}` }))]} onChange={(referenceId) => updateSpecialist(row.clientId, { referenceId })} /></Field>
          ...
          <div className="entrance-rule-editor">
            {draft.rules.map((row, index) => <article key={row.clientId} className={ruleIssues[index] ? "is-invalid" : ""}>
              ...
              <div className="entrance-rule-fields">
                ...
                <Field label="结果路由"><SelectControl ariaLabel={`选择规则 ${row.id || index + 1} 的结果路由`} value={row.route} invalid={Boolean(ruleResultIssues[index])} options={[{ value: "direct", label: "Direct · 直达", description: "使用固定 Direct 目标" }, { value: "specialist", label: "Specialist · 专家", description: "转给一个具名专家目标" }, { value: "leader", label: "Leader · 协作编排", description: "启动领队团队拆解与分工" }]} onChange={(route) => updateRule(row.clientId, { route: route as DecisionRoute, specialistKey: route === "specialist" ? row.specialistKey : "" })} /></Field>
                {row.route === "specialist" && <Field label="Specialist Key"><SelectControl ariaLabel={`选择规则 ${row.id || index + 1} 的专家目标`} value={row.specialistKey} invalid={Boolean(ruleResultIssues[index])} options={[{ value: "", label: "选择专家目标", disabled: true }, ...specialistKeys.map((key) => ({ value: key, label: key }))]} onChange={(specialistKey) => updateRule(row.clientId, { specialistKey })} /></Field>}
              </div>
        ...
        <section className="workflow-contract"><div className="section-kicker"><b>04</b><span>兜底结果</span></div><div className="form-grid entrance-default-grid">
          <Field label="Default Route"><SelectControl ariaLabel="选择默认路由" value={draft.defaultRoute} options={[{ value: "direct", label: "Direct · 直达", description: "使用固定 Direct 目标" }, { value: "specialist", label: "Specialist · 专家目标", description: "转给默认专家目标" }, { value: "leader", label: "Leader · 协作编排", description: "启动领队团队" }]} onChange={(defaultRoute) => setDraft({ ...draft, defaultRoute: defaultRoute as DecisionRoute, defaultSpecialistKey: defaultRoute === "specialist" ? draft.defaultSpecialistKey : "" })} /></Field>
          {draft.defaultRoute === "specialist" && <Field label="Default Specialist Key"><SelectControl ariaLabel="选择默认专家目标" value={draft.defaultSpecialistKey} invalid={Boolean(defaultIssue)} options={[{ value: "", label: "选择专家目标", disabled: true }, ...specialistKeys.map((key) => ({ value: key, label: key }))]} onChange={(defaultSpecialistKey) => setDraft({ ...draft, defaultSpecialistKey })} /></Field>}
...
function EvaluationDesk({ policy, data, notify }: { policy: EntrancePolicy; data: Bootstrap; notify: PageProps["notify"] }) {
  ...
  return <div className="entrance-evaluation-desk">
    ...
    {intent === "employee" && <div className="intent-target-picker"><Field label="选择执行目标"><SelectControl ariaLabel="选择单员工执行目标" value={specialistKey} invalid={!specialistKey} options={[{ value: "", label: "选择一位员工或单目标流程", disabled: true }, ...specialistEntries.map(([key, target]) => ({ value: key, label: target.kind === "employee" ? data.employees.find((employee) => employee.id === target.employeeId)?.identity.displayName ?? target.employeeId : targetLabel(target), description: key }))]} onChange={setSpecialistKey} /></Field>{specialistTarget && <p>{specialistName ?? targetLabel(specialistTarget)} · 内部固定版本已由策略保存</p>}</div>}
    ...
    <details className="entrance-advanced"><summary><span>高级启动规则</span><small>系统集成与确定性路由调试</small><UtilityIcon name="toggle" /></summary><div className="entrance-evaluator-grid">
      <Field label="内部路由"><SelectControl ariaLabel="选择高级试算的内部路由" value={route} options={[{ value: "auto", label: "Auto · 按策略", description: "依次执行顺序规则与兜底结果" }, { value: "direct", label: "Direct", description: "强制使用 Direct 目标" }, { value: "specialist", label: "Specialist", description: "强制使用指定专家目标" }, { value: "leader", label: "Leader", description: "强制使用领队协作编排" }]} onChange={(nextRoute) => setRoute(nextRoute as EntrancePolicyRoute)} /></Field>
      {route === "specialist" && <Field label="目标 Key"><SelectControl ariaLabel="选择高级试算的专家目标" value={specialistKey} invalid={!specialistKey} options={[{ value: "", label: "选择目标", disabled: true }, ...specialistEntries.map(([key]) => ({ value: key, label: key }))]} onChange={setSpecialistKey} /></Field>}
      <Field label="任务标签"><input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="review, risky" /></Field>
      <Field label="调用来源"><SelectControl ariaLabel="选择高级试算的调用来源" value={sourceKind} options={[{ value: "workbench", label: "Workbench" }, { value: "http", label: "HTTP" }, { value: "mcp", label: "MCP" }, { value: "a2a", label: "A2A" }]} onChange={(nextSource) => setSourceKind(nextSource as InvocationSourceKind)} /></Field>
