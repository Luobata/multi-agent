...
import { DEFAULT_EMPLOYEE_ACCENT, DossierSection, EmptyState, Field, Modal, Stamp, UtilityIcon, formatTime, initials, scrollRecordIntoView, useDaemonAvailable } from "./components";
...
export function WorkflowPage({ data, refresh, notify }: PageProps) {
  ...
  return <div className="page-grid">
    ...
    <main className="detail-pane">{!selected ? <EmptyState title="用常用架构建立第一份编排" action={<button className="button primary" disabled={!daemonAvailable} onClick={() => setEditor("new")}>选择架构模板</button>}>从顺序流水线、并行汇总、评审委员会或计划执行模板开始，映射员工后可在画布继续拖动和修改依赖。</EmptyState> : <div className="dossier workflow-dossier">
      ...
      <DossierSection number="02" title="节点清册"><div className="node-ledger">{selected.nodes.map((node, index) => { const employee = data.employees.find((item) => item.id === node.employeeId); return <article key={node.id}><span className="node-number">{String(index + 1).padStart(2, "0")}</span><span className="employee-initials small" style={{ "--accent": employee?.presentation.accent ?? DEFAULT_EMPLOYEE_ACCENT } as CSSProperties}>{initials(employee?.identity.displayName ?? node.employeeId, employee?.presentation.initials)}</span><div><strong>{node.id}</strong><span>{employee?.identity.displayName ?? node.employeeId} · v{node.employeeVersion ?? employee?.version ?? "—"}</span></div><code>{node.needs.length ? `← ${node.needs.join(", ")}` : "并行起点"}</code></article>; })}</div></DossierSection>
