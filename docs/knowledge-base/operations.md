...
| 知识目录 | KnowledgeBase、Collection、Source、人工 Document、URL 导入入口与目录元数据 | 不给员工逐库授权 |
| 全量 Wiki | 按知识库、Collection 和文档树浏览发布版或最新草稿，查看来源、显式引用、反向引用和有限候选 | 不自动固化候选关系 |
| 发布车道 | Revision 质检、草稿检索试跑、影响确认、发布与回滚 | 不修改历史 Run |
| 知识 Profile | 多规则目录范围、信任边界、激活条件和单次预算 | 不保存正文和索引 |
| 影响与授权 | 解释 KnowledgeBase → Profile → Employee / Project Role 的确定性关系 | 不使用模型推断权限 |
| 授权复核 | 按逾期、临期、正常、未排期查看授权，生成保留、收窄或撤销提案 | 不自动改权 |
...
```text
选择目标 KnowledgeBase / Collection 与 URL
  → 服务端受限抓取并拆成结构化文档
  → 检查最终 URL、跳转、字节数、正文、层级和冻结哈希
  → 从最多 5 个弱关系候选中人工选择，并指定关系类型
  → 生成 KnowledgeChangeRequest
  → 人工批准后生成未发布 Revision
  → 质检与试跑
  → 显式发布
```

预览不写入知识；提案前会重新抓取，页面内容或目标知识库版本变化都会要求重新预览。重复导入同一页面时，新草稿替换该页面原有文档，保留稳定来源位置和既有显式引用；不会把旧页面与新页面同时堆叠。

关系维护遵循两条规则：

- 导入时写入来源身份、父子层级，以及维护者确认的强关系；
- “可能相关”的语义关系保持为按需候选，等确实被使用或人工复核时再提交为显式关系。

因此不会在每次新增文档时扫描并永久连接所有旧文档。候选固定限额，知识库膨胀主要增加搜索成本，不会自动膨胀为完整关系网。
...
员工详情 → 调整知识授权
  → 勾选少量 Profile
  → 填写理由、授权负责人和可选的到期/复核周期
  → 生成 employee-profiles.set 提案
  → 人工查看影响并批准
  → 新 Employee 版本生效；已有 Session 继续固定旧版本
```

员工档案的身份、提示词或 Skill 编辑不携带知识授权字段，避免普通档案修订绕过审批。新增授权的元数据逐条记录；移除或新增某一 Profile 不会改写其他保留授权的理由和时间。
...
| 授权即将到期 | 在“授权复核”选择保留、收窄或撤销，生成提案并交由人批准 |
...
- **授权审批人**确认理由、影响、期限和复核安排；系统台账只提醒，不替人做决定。
