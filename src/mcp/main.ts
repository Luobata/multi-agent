...
program
  ...
  .option("--profile <profile>", "tool profile: full, knowledge-control, configuration-control, or gate-control", "full")
  ...
  .action(async (options: { daemonUrl: string; profile: string; sourceRunId?: string }) => {
    if (
      ...
      && options.profile !== "knowledge-control"
      && options.profile !== "configuration-control"
      && options.profile !== "gate-control"
    ) {
      throw new Error("profile must be full, knowledge-control, configuration-control, or gate-control");
    }
