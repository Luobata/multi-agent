...
const TEAM_ORCHESTRATION_SKILL_VERSION = 5;
const TEAM_ORCHESTRATION_INSTRUCTIONS = [
  ...
  "Before delegating test or audit work, derive independent validation domains from acceptance criteria, changed files, UI routes, APIs, persistence boundaries, and compatibility risks. Treat a scope as oversized when it spans more than three independent domains or combines browser validation with repository-wide test, typecheck, and build. Split oversized validation into two or three explicit assignments to the same testing role, keep the same changeSet, give each assignment a bounded evidence scope and stop condition, and dispatch them in parallel within maxParallelDelegations. Later gates must reuse same-commit shard evidence and run only the smallest missing cross-shard check; never ask one tester to repeat every shard or debug the test infrastructure indefinitely.",
